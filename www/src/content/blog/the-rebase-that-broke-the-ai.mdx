---
title: "The Rebase That Broke the AI: Why Stacked Diffs Need Topology Awareness"
description: "A real conversation with Claude Code exposed a fundamental problem with manual rebasing of stacked branches. Here's the story, the test that captures it, and why git rebase --onto knowledge shouldn't live in your head."
...
A real conversation with Claude Code exposed exactly why manual rebasing of stacked branches is so error-prone &mdash; and how SDF eliminates the problem.
...
That's it. SDF tracks each branch's `BaseTip` &mdash; the SHA of the parent branch at the time the child was last synced. When it detects that the parent has moved, it computes the `--onto` rebase automatically:
...
The engine branch gets new commits (simulating review feedback). This is the trigger &mdash; the wiring branch's `BaseTip` now points at the old engine tip, but the engine has moved forward.
...
Each of those is a decision point where things can go wrong. The `--onto` boundary commit is especially dangerous &mdash; pick the wrong one and you either lose commits or duplicate them.
...
Stacked branches create topology relationships that plain git doesn't track. When an upstream branch is rewritten, every downstream branch needs a topology-aware rebase &mdash; not a naive one. The difference is `git rebase` (which replays shared history and explodes) vs `git rebase --onto` (which replays only branch-specific commits).
