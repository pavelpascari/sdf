...
```
                    ...
                    ├─────────────────────────────────────────────────────┤
                    │                                                     │
  Confidence ▲      │   Layer 5 ── Build Tags                            │
             │      │       Isolate test infra from production binary     │
             │      │                                                     │
             │      │   Layer 4 ── Cross-Validation                      │
             │      │       Prove fakes match real tool output            │
             │      │                                                     │
             │      │   Layer 3 ── Spy Recording                         │
             │      │       Capture what the real tools actually return   │
             │      │                                                     │
             │      │   Layer 2 ── Binary Injection                      │
             │      │       Swap real binaries for fakes in tests         │
             │      │                                                     │
             │      │   Layer 1 ── Fake Binaries                         │
             │      │       Shell scripts that pretend to be git/gh      │
             │      │                                                     │
             └──────┴─────────────────────────────────────────────────────┘
```
...
```
  Test                          Fake Binary                    Log File
   ...
   │      ["pr list --state all"]                                 │
   │                                                              │
   ▼  Assert result + Assert invocations                          │
```
...
```
  ...
  var Binary = "gh"                    SetBinary(t, &Binary, "/tmp/fake-gh")
       │                                    │
       ▼                                    ▼
  exec.Command("gh", ...)             exec.Command("/tmp/fake-gh", ...)
       │                                    │
       ▼                                    ▼
  ┌─────────────┐                     ┌─────────────────┐
  │   real gh    │                     │   fake gh       │
  │  (via PATH)  │                     │  (shell script) │
  └─────────────┘                     └─────────────────┘
       │                                    │
       ▼                                    ▼
  GitHub API                           Canned JSON + log file
                                            │
                                       t.Cleanup restores
                                       Binary = "gh"
```
...
```
  ...
  ──────────                              ─────────
  Use fake binaries                       Use real git & gh
       │                                       │
       ▼                                       ▼
  ┌──────────────────┐                ┌──────────────────┐
  │  Fake Responses  │                │    Recordings    │
  │                  │                │                  │
  │  "pr list" ──►   │                │  "pr list" ──►   │
  │  [{"number":10,  │                │  [{"number":847, │
  ...
  └────────┬─────────┘                └────────┬─────────┘
           ...
           │          JSONShape()               │
           ▼                                   ▼
    array[object{         ◄── compare ──►  array[object{
      ...
      url}]                                  url}]
           ...
           └───────────── match? ──────────────┘
                            │
                     ┌──────┴──────┐
                     │  ✓  same    │  Fakes are honest
                     │  ✗  differ  │  Fakes have drifted
                     └─────────────┘
```
...
```
  ...
  ┌───────────────────┐
  │ Unit tests        │  milliseconds     every save       fast
  │ with fakes        │                                    feedback
  └─────────┬─────────┘
            │
  ┌─────────▼─────────┐
  │ Cross-validation  │  seconds          periodic         fakes are
  │ shape comparison  │                                    honest
  └─────────┬─────────┘
            │
  ┌─────────▼─────────┐
  │ Spy recordings    │  seconds          on demand        ground
  │ capture reality   │                                    truth
  └─────────┬─────────┘
            │
  ┌─────────▼─────────┐
  │ E2E tests         │  minutes          in CI            real
  │ real git, real gh  │                                    workflows
  └───────────────────┘

  ◄──────────────────────────────────────────────────────────────────►
  Cheap, fast, frequent                     Expensive, slow, thorough
```
