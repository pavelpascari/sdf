...
```mermaid
graph BT
    L1["<b>Layer 1: Fake Binaries</b><br/>Shell scripts that pretend to be git/gh"]
    L2["<b>Layer 2: Binary Injection</b><br/>Swap real binaries for fakes in tests"]
    L3["<b>Layer 3: Spy Recording</b><br/>Capture what the real tools actually return"]
    L4["<b>Layer 4: Cross-Validation</b><br/>Prove fakes match real tool output"]
    L5["<b>Layer 5: Build Tags</b><br/>Isolate test infra from production binary"]
...
graph TD
    ...
    subgraph Tests
        TB["SetBinary(t, &Binary, fakePath)"]
        TE["exec.Command(Binary, args...)"]
        TR(["fake gh — shell script"])
        TA["Canned JSON + log file"]
        TC>"t.Cleanup restores Binary = 'gh'"]
        TB --> TE --> TR --> TA
        TA -.-> TC
    end
```
...
```mermaid
graph TD
    UT["Unit Tests<br/>use fake binaries"] --> FR["Fake Responses<br/>number: 10, state: OPEN, ..."]
    E2E["E2E Tests<br/>use real git & gh"] --> REC["Recordings<br/>number: 847, state: OPEN, ..."]

    FR -->|"JSONShape()"| FS["array[object{headRefName,<br/>number, state, url}]"]
    REC -->|"JSONShape()"| RS["array[object{headRefName,<br/>number, state, url}]"]
...
```mermaid
graph TD
    U["<b>Unit tests with fakes</b><br/>Milliseconds · every save · fast feedback"]
    CV["<b>Cross-validation</b><br/>Seconds · periodic · fakes stay honest"]
    SPY["<b>Spy recordings</b><br/>Seconds · on demand · ground truth"]
    E2E["<b>E2E tests</b><br/>Minutes · in CI · real workflows"]
